...
If the business suddenly asked us to track 15 new metrics (CPU usage, memory, temperature, network bandwidth, error counts, etc.), adding 15 new maps and slices to `DeviceData` would bloat the code, violate the Open-Closed principle, and require constant API refactoring.
...
Where `D` is unique devices, `H` is heartbeats, and `U` is upload events. The memory usage grows linearly indefinitely because there isn't a TTL (Time-To-Live) eviction policy. This is perfectly acceptable for the assignment constraints, but would intuitively crash a production server over several months unless pushed to a cold-storage database.
